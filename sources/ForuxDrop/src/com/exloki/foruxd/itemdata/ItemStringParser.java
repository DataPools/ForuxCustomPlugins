...
public class ItemStringParser {
    private static final String LOG_PREFIX = "[Crates] ";
    ...
    public ItemStack parse(boolean deepLog) {
        ...
        if (dataMap.isEmpty()) {
            ForuxDrop.i.log("Disregarding invalid item string: '" + input + "'");
            errors = true;
            return null;
        }
        ...
        try {
            ...
            ForuxDrop.i.log("Disregarding invalid item string: '" + input + "' - " + ex.getMessage());
            errors = true;
            return null;
        }
        ...
        for (ItemPart rel : dataMap.keySet()) {
            ...
            for (String value : dataMap.get(rel)) {
                try {
                    ...
                    ForuxDrop.i.log("Disregarding invalid ItemPart with value '" + value + "' - " + ex.getMessage());
                    errors = true;
                }
            }
        }
    ...
    protected HashMap<ItemPart, List<String>> createPartMap(boolean deepLog) {
        ...
        for (String str : set) {
            if (!str.contains(EQUALIZER) || input.indexOf(EQUALIZER) == input.length()) {
                ForuxDrop.i.log("Disregarding invalid ItemPart '" + str + "' as no value can be found");
                errors = true;
                continue;
            }
            ...
            if (part == null) {
                if (deepLog)
                    ForuxDrop.i.log("Disregarding invalid ItemPart definition: '" + key + "'");
                continue;
            }
