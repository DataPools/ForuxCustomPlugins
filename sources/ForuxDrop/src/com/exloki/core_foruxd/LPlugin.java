package com.exloki.core_foruxd;

import com.exloki.core_foruxd.commands.ICommand;
import com.exloki.core_foruxd.config.PluginWrapper;
import com.exloki.core_foruxd.exceptions.InvalidArgumentsException;
import com.exloki.core_foruxd.exceptions.LException;
import com.exloki.core_foruxd.exceptions.PlayerNotFoundException;
import com.exloki.core_foruxd.utils.Persist;
...
public class LPlugin extends PluginWrapper {
    ...
    public LPlugin(String messsagePrefix, String cmdPermPrefix, String cmdClassPath) {
        this.gson = this.getGsonBuilder().create();
        this.persist = new Persist(this);

        this.messagePrefix = messsagePrefix;
        this.commandClassPath = cmdClassPath;
        this.commandPermissionPrefix = cmdPermPrefix;
    ...
    private String commandPermissionPrefix, commandClassPath;

    @Override
    public boolean onCommand(final CommandSender sender, final Command command, final String commandLabel, final String[] args) {
        return onLCommand(sender, command, commandLabel, args, LPlugin.class.getClassLoader(), commandClassPath, commandPermissionPrefix);
    }

    public boolean onLCommand(final CommandSender cSender, final Command command, final String commandLabel, final String[] args, final ClassLoader classLoader, final String commandPath, final String permissionPrefix) {
        try {
            ...
            if (!isConsole) {
                if (!cSender.hasPermission(permissionPrefix + "." + command.getName())) {
                    cSender.sendMessage(TL.ER_PERMS.toString());
                    return true;
                }
            }
            ...
            try {
                ...
                if (!ex.getMessage().isEmpty()) {
                    cSender.sendMessage(TL.ER_USAGE.withVars(ex.getMessage()));
                    return true;
                }

                if (!cmd.getUsage().equals("*unavailable*")) {
                    cSender.sendMessage(TL.ER_USAGE.withVars(cmd.getUsage()));
                    return true;
                }

                cSender.sendMessage(TL.ER_ERROR.withVars("Invalid command arguments"));
                return true;
            } catch (PlayerNotFoundException ex) {
                cSender.sendMessage(TL.ER_PLAYER.toString());
                return true;
            } catch (LException ex) {
                cSender.sendMessage(ex.getMessage());
                return true;
            } catch (Exception ex) {
                cSender.sendMessage(TL.ER_ERROR.withVars("An internal error has occured. Please contact an admin immediately!"));
                ex.printStackTrace();
                return true;
            }
        } catch (Throwable ex) {
